...
const PIPELINE_VERSION = `task-v5:${TEXT_MODEL}:${VISION_MODEL}:${path.basename(process.env.WHISPER_MODEL_PATH || DEFAULT_WHISPER_MODEL)}`;
...
async function ollama(
  ...
  try {
    ...
    throw new Error(`Cannot connect to Ollama at ${OLLAMA_URL}. Start it with: ollama serve`);
  }
  if (!response.ok) {
...
function hasTaskIntent(value: string): boolean {
  ...
  const hindiCommitment = /(?:मुझे|हमें|मुझको|हमको).{0,100}(?:करना है|जाना है|जाना है|भेजना है|मिलना है|खरीदना है|याद रखना है)|(?:याद दिला(?:ना|दो)|रिमाइंड)|(?:कल|आज|परसों).{0,100}(?:जाना है|करना है|भेजना है|मिलना है|खरीदना है|ऑफिस)/u.test(text);
  const romanHindiCommitment = /\b(?:mujhe|humein|humko|mujhko).{0,100}\b(?:karna|jana|jaana|bhejna|milna|khareedna|kharidna|yaad)\b|\b(?:kal|aaj|parso).{0,100}\b(?:karna|jana|jaana|bhejna|milna|khareedna|kharidna|office)\b|\b(?:yaad dila(?:na|dena)?|remind kar(?:na|dena)?)\b/i.test(text);
...
function modelReminders(candidates: ReminderCandidate[], source: ReviewSource): AiReminder[] {
  ...
  for (const candidate of candidates) {
    ...
    if (!sourceContent.includes(normalized(evidence)) || !hasTaskIntent(evidence)) continue;
...
async function transcribeAudio(input: string): Promise<string> {
  ...
  try {
    try {
      await execFileAsync(process.env.FFMPEG_BIN || 'ffmpeg', ['-y', '-i', input, '-vn', '-ac', '1', '-ar', '16000', wav], { maxBuffer: 1024 * 1024 });
    } catch {
      throw new Error('FFmpeg could not decode this media. Install it with: brew install ffmpeg');
    }
    try { await fs.access(whisperModel); } catch {
      throw new Error(`Whisper speech model is missing at ${whisperModel}. Set WHISPER_MODEL_PATH to a multilingual GGML model.`);
    }
    try {
      await execFileAsync(process.env.WHISPER_BIN || 'whisper-cli', [
        '-m', whisperModel, '-l', 'auto', '-f', wav, '-otxt', '-of', outputBase,
      ], { maxBuffer: 1024 * 1024 });
    } catch {
      throw new Error('Whisper.cpp could not transcribe this media. Install it with: brew install whisper-cpp');
    }
...
async function analyzeVideo(filePath: string): Promise<{ content: string; warnings: string[] }> {
  ...
  try {
    ...
    try {
      ...
      await execFileAsync(process.env.FFMPEG_BIN || 'ffmpeg', ['-y', '-i', filePath, '-vf', 'fps=1/2,scale=1024:-1', '-frames:v', '4', pattern], { maxBuffer: 1024 * 1024 });
